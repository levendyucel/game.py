...
# Kullanıcıya başlık ve açıklama
st.title("🃏 Kelime Kartları Oyunu")
st.write("Kelimenin üzerine tıklayın ve arkasındaki tanımı keşfedin!")

# Kartlar konteynerini oluştur
...
for word, definition in sozluk.items():
    ...
    # Kartın önü ve arkası, tıklayınca değişim
    if st.button(f"{word}", key=button_id):
        st.session_state.flipped_cards[word] = not card_flipped

    # Kartın ön tarafı mı yoksa arka tarafı mı gözükmeli
